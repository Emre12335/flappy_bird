# Ses dosyası ekleme
# pygame.mixer.Sound() ile variable oluşturup
# variable ı play_sound() ile oynatmalıyız.
...
def check_collisions(pipes: list):  # 1 game active check
    for pipe in pipes:
        if animated_bird_r.colliderect(pipe):
            death_sound.play()
            return False
    new_leftx_list = {pipe.left for pipe in pipe_list}
    for value in new_leftx_list:
        old = value
        new = value - 5
        if new < animated_bird_r.right <= old and animated_bird_r.bottom <= 0:
            death_sound.play()
            return False
